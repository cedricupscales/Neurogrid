...
export const SynapseGrid = ({ tasks, onRefresh }: { tasks: Task[], onRefresh: () => void }) => {
  ...
  return (
    <div className="flex flex-col h-full gap-6">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-bold uppercase tracking-[0.3em] text-white/30">Synapse Map</h3>
          ...
          return (
            <div key={day.toString()} className="flex flex-col gap-4 h-full">
              ...
              <div className="flex-1 bg-black/20 rounded-2xl border border-white/5 p-2 overflow-y-auto custom-scrollbar flex flex-col gap-2">
                {dayTasks.map(task => (
                  <Card key={task.id} className="p-2 border-l-2 border-l-[#22C55E] bg-[#1A1A1A]">
                ...
                {dayTasks.length === 0 && (
                  <div className="flex-1 border border-dashed border-white/5 rounded-xl flex items-center justify-center opacity-20">
                    <span className="text-[8px] uppercase tracking-tighter">Quiet</span>
                  </div>
                )}
              </div>
            </div>
